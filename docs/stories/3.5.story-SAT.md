...
**Command**:
```bash
cat packages/poem-core/tasks/validate-prompt.yaml | head -20
```
...
**Validation**:
- ✅ File exists at `packages/poem-core/tasks/validate-prompt.yaml`
- ✅ Contains `id: validate-prompt`
- ✅ Contains `pathResolution: config`
- ✅ Has workflow header comments explaining execution model
...
**Command**:
```bash
grep -E "^  - id:" packages/poem-core/tasks/validate-prompt.yaml
```
...
**Validation**:
- ✅ Exactly 12 step IDs present
- ✅ Step IDs follow kebab-case naming
- ✅ Steps cover: select, load, validate (syntax/schema/helpers), best practices, aggregate, report, save, complete
...
**Command**:
```bash
grep -E "^paths:" packages/poem-core/tasks/validate-prompt.yaml
```
...
**Validation**:
- ✅ No output from grep (no paths: section found)
- ✅ Workflow uses `pathResolution: config` instead
...
```
 ...
 ✓ tests/workflows/validate-prompt.test.ts (42 tests) 10ms
...
- ✅ All 42 tests passed
- ✅ No test failures
- ✅ Test file: `tests/workflows/validate-prompt.test.ts`
- ✅ Test categories: YAML structure, step validation, AC coverage, path resolution
...
**Validation**:
- ✅ Test file exists at `packages/poem-app/tests/workflows/validate-prompt.test.ts`
- ✅ Uses Vitest framework
- ✅ Loads and parses workflow YAML file
- ✅ Contains describe block for "validate-prompt workflow"
...
**Command**:
```bash
wc -l packages/poem-core/tasks/validate-prompt.yaml
```

**Expected Output**:
```
     712 packages/poem-core/tasks/validate-prompt.yaml
```

**Validation**:
- ✅ File is approximately 700+ lines
- ✅ Contains comprehensive step instructions (not just placeholders)
...
**How to test**: Run `/poem/agents/penny` → `*validate {prompt-name}` → Observe syntax validation step
...
**When**: Story 3.6+ or when using Prompt Engineer agent workflows
**How to test**: Run `/poem/agents/penny` → `*validate {prompt-name}` → Observe schema validation steps
...
**When**: Story 3.6+ or when using Prompt Engineer agent workflows
**How to test**: Run `/poem/agents/penny` → `*validate {prompt-name}` → Observe helper validation step
...
**When**: Story 3.6+ or when using Prompt Engineer agent workflows
**How to test**: Run `/poem/agents/penny` → `*validate {prompt-name}` → Observe best practice check step
...
**When**: Story 3.6+ or when using Prompt Engineer agent workflows
**How to test**: Run `/poem/agents/penny` → `*validate {prompt-name}` → Observe severity categorization in report
...
**When**: Story 3.6+ or when using Prompt Engineer agent workflows
**How to test**: Run `/poem/agents/penny` → `*validate {prompt-name}` → Observe fix suggestions in report
...
ls packages/poem-core/tasks/validate-prompt.yaml
...
ls -la packages/poem-core/tasks/validate-prompt.yaml

...
- [x] Task 2: Implement helper add detection (AC: 2, 5)
  ...
  - [x] Log: `[HotReload] ➕ Added helper: {name}`
...
- [x] Task 3: Implement helper modify detection (AC: 3, 5)
  ...
  - [x] Log: `[HotReload] 🔄 Reloaded helper: {name}`
...
- [x] Task 4: Implement helper delete detection (AC: 4, 5)
  ...
  - [x] Log: `[HotReload] ➖ Removed helper: {name}`
...
- [x] Task 5: Implement error handling for invalid helpers (AC: 6)
  ...
  - [x] Log: `[HotReload] ❌ Error loading {name}: {error.message}`
...
- [x] Task 6: Integrate watcher with server startup
  ...
  - [x] Log watcher status on startup: `[HotReload] 👀 Watching for helper changes...`
...
│   ├── index.ts           ← HandlebarsService (existing)
│   ├── loader.ts          ← Static loader (existing, uses import.meta.glob)
│   ├── watcher.ts         ← CREATE: HelperWatcher class
│   └── helpers/           ← Watch this directory
│       ├── *.js           ← Helper files to watch
│       └── *.d.ts         ← Type declarations (ignore in watcher)
│
└── tests/services/handlebars/
    └── watcher.test.ts    ← CREATE: Watcher tests
```
...
**Critical Rule:** "New helpers must be valid JavaScript modules. Invalid syntax must not crash the server — log and skip."
...
- Coding Standards: ✓ Follows helper hot-reload safety rule (invalid syntax logs and skips, doesn't crash)
- Project Structure: ✓ Files in correct locations (watcher.ts in services/handlebars, tests in tests/services/handlebars)
- Testing Strategy: ✓ 18 unit tests with vitest, comprehensive coverage of all ACs
...
Gate: **PASS** → docs/qa/gates/2.4-hot-reload-helpers.yml

### Recommended Status

✓ **Ready for Done** - All acceptance criteria met, tests passing, code follows standards.

...
- [x] Task 1: Create validate-prompt.yaml Workflow File (AC: 1)
  - [x] Create `packages/poem-core/tasks/validate-prompt.yaml`
...
- [x] Task 11: Implement Step 10 - Display Validation Report (AC: 6, 7)
  ...
  - [x] If PASS, display "✓ All validation checks passed"
  - [x] If FAIL, display "✗ Validation failed with X error(s)"
...
[Source: packages/poem-core/tasks/test-prompt.yaml]
...
**Workflow File:**
- Location: `packages/poem-core/tasks/validate-prompt.yaml`
- Naming: kebab-case
...
- Missing schema files → Continue validation, note schemas skipped (info level)
- API errors → Show error message with context, offer retry
- Invalid file paths → Display error, list available prompts
...
├── refine-prompt.yaml       # ✅ Implemented (Story 3.3)
├── test-prompt.yaml         # ✅ Implemented (Story 3.4)
├── validate-prompt.yaml     # 🆕 This story
...
├── refine-prompt.yaml
├── test-prompt.yaml
├── validate-prompt.yaml     # 🆕 Deployed here when installed
└── ...
```
...
**Created:**
- `packages/poem-core/tasks/validate-prompt.yaml` - Main workflow definition (712 lines, 12 steps)
...
- **Coding Standards:** ✅ PASS
  - Kebab-case naming (validate-prompt.yaml) ✓
  - YAML structure matches architectural spec ✓
  - No hardcoded paths (uses pathResolution: config) ✓
  - Comprehensive header comments ✓

- **Project Structure:** ✅ PASS
  - Workflow file: `packages/poem-core/tasks/validate-prompt.yaml` ✓
  - Test file: `packages/poem-app/tests/workflows/validate-prompt.test.ts` ✓
  - Locations match unified-project-structure.md ✓

- **Testing Strategy:** ✅ PASS
  - 42 unit tests for workflow structure validation ✓
  - Tests validate YAML parsing, step structure, AC coverage ✓
  - All tests passing (42/42) ✓
  - Follows Vitest patterns from testing-strategy.md ✓

- **All ACs Met:** ✅ PASS
  - AC1: Workflow defined in validate-prompt.yaml ✓
  - AC2-7: Workflow steps implement all validation logic ✓
  - Unit tests verify AC coverage ✓
...
|-----|-------------|---------------|--------|
| AC1 | Workflow defined in `.poem-core/workflows/validate-prompt.yaml` | 8 tests validate YAML structure, metadata, pathResolution | ✅ PASS |
| AC2 | Validates Handlebars syntax | Step 3 (validate-syntax) in workflow calls Handlebars.compile() | ✅ PASS |
| AC3 | Validates input/output schema alignment | Steps 5-7 (load-schemas, validate-input-schema) check placeholders vs schemas | ✅ PASS |
| AC4 | Validates required helpers | Step 8 (validate-helpers) calls /api/helpers and compares | ✅ PASS |
| AC5 | Checks POEM best practices | Step 9 (check-best-practices) applies 4 configurable rules | ✅ PASS |
| AC6 | Reports issues with severity | Steps 10-11 (aggregate-results, validation-report) categorize by error/warning/info | ✅ PASS |
| AC7 | Provides fix suggestions | Step 10 (aggregate-results) generates actionable suggestions | ✅ PASS |
...
**Gate:** PASS → docs/qa/gates/3.5-validate-prompt-workflow.yml
...
- **Pattern**: [Skills Self-Description Format](../kdd/patterns/skills-self-description-format.md) — Skills define their own interface in SKILL.md using a structured markdown format; agents discover capabilities by reading the skill file
- **Decision**: [ADR-004: Skills as Markdown Documentation](../kdd/decisions/adr-004-skills-as-markdown-documentation.md) — Skills are plain markdown files rather than code modules; agent interprets and executes them, enabling non-programmer skill authoring

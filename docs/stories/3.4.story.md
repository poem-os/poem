...
- [x] Task 1: Create test-prompt.yaml Workflow File (AC: 1)
  - [x] Create `packages/poem-core/workflows/test-prompt.yaml`
...
**Workflow File:**
- Location: `packages/poem-core/workflows/test-prompt.yaml`
- Naming: kebab-case
...
[Source: packages/poem-core/workflows/refine-prompt.yaml § Step 9-10]
...
- Missing schema files → Continue without schema validation (warn user)
- Missing mock data → Prompt user for inline data or file path
- API errors → Show error message with context, offer retry
- Invalid JSON data → Display parse error, prompt again
...
├── new-prompt.yaml
├── refine-prompt.yaml
├── test-prompt.yaml         # 🆕 Deployed here when installed
└── ...
```
...
- Step 3 (load-test-data) → Step 4 (load-schema) → Step 5 (render-template) → Step 6 (validate-output) → Step 7 (collect-results) → Step 8 (run-another-scenario)
...
**Data Accumulation:**
- Scenario 1 results → stored in testResults[0]
- Scenario 2 results → stored in testResults[1]
...
1. **Workflow File** (`packages/poem-core/workflows/test-prompt.yaml`):
...
**Created:**
- `packages/poem-core/workflows/test-prompt.yaml` - Main workflow definition (523 lines)
...
|----|-------------|----------------|---------------|--------|
| 1 | Workflow file definition | test-prompt.yaml (523 lines) | 41 unit tests | ✅ Complete |
| 2 | Data source flexibility | Steps 1-3 (select, choose, load) | Structure validated | ✅ Complete |
| 3 | Render API + validation | Steps 5-6 with API calls | API refs validated | ✅ Complete |
| 4 | Error reporting | Steps 5-6 + fallback config | Instructions validated | ✅ Complete |
| 5 | Multiple scenarios | Step 8 loop-back pattern | Step structure validated | ✅ Complete |
| 6 | Metrics display | Step 5 captures renderTimeMs + length | Instruction content validated | ✅ Complete |
| 7 | File saving | Step 10 with user confirmation | Elicit pattern validated | ✅ Complete |
...
**Next Steps:**
1. Story automatically closed (status → Done)

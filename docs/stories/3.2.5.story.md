...
- [x] Task 5: Remove Path Duplication from new-prompt.yaml (AC: 6, 7)
  - [x] Modify `packages/poem-core/tasks/new-prompt.yaml`
...
- [x] Task 6: Document Pattern for Future Workflows (AC: 11)
  - [x] Create `packages/poem-core/tasks/README.md` with:
...
When running `/poem/agents/penny` and executing `*new` in development mode, files are created at:
...
3. Follow the pattern documented in `packages/poem-core/tasks/README.md`
...
```bash
grep -A5 "^paths:" packages/poem-core/tasks/new-prompt.yaml || echo "No paths section found (correct)"
```
...
```bash
head -10 packages/poem-core/tasks/README.md
```
...
- `packages/poem-core/tasks/README.md` - Workflow pattern documentation
...
- `packages/poem-core/tasks/new-prompt.yaml` - Removed paths section, added pathResolution: config
...
**Patterns**:
- [Config Service Single Source of Truth](../../kdd/patterns/config-service-single-source-of-truth.md) - Established `poem-core-config.yaml` as source of truth; workflows inherit paths from config
...
**ADRs**:
- [ADR-002: Multi-Workflow Architecture](../../kdd/decisions/adr-002-multi-workflow-architecture.md) - Config service enables workspace isolation for multi-workflow scenarios

...
```bash
ls -lh packages/poem-core/tasks/test-prompt.yaml
```

**Expected Output**:

```
-rw-r--r--  1 user  staff   523B  Jan 11 20:41 packages/poem-core/tasks/test-prompt.yaml
```

**Validation**:

- ✅ File exists at `packages/poem-core/tasks/test-prompt.yaml`
- ✅ File size is approximately 523 bytes (520-530 bytes acceptable)
...
```
✓ tests/workflows/test-prompt.test.ts (41 tests) 8ms
...
- ✅ All required fields present (id, name, version, pathResolution, etc.)
- ✅ All 11 steps have unique IDs
- ✅ All 7 acceptance criteria covered by steps
...
```bash
grep -A 5 "^id:" packages/poem-core/tasks/test-prompt.yaml | head -n 6
```
...
- ✅ Workflow id is "test-prompt"
- ✅ Version is "1.0.0"
- ✅ Author is "POEM Framework"
- ✅ lastUpdated is "2026-01-11"
...
```bash
grep "pathResolution:" packages/poem-core/tasks/test-prompt.yaml
```
...
- ✅ pathResolution is set to "config"
- ✅ Workflow inherits paths from poem-core-config.yaml
...
```bash
grep -c "^  - id:" packages/poem-core/tasks/test-prompt.yaml
```
...
```bash
grep "^  - id:" packages/poem-core/tasks/test-prompt.yaml | awk '{print $3}'
```
...
- ✅ All 11 step IDs present
- ✅ Step IDs match workflow specification
- ✅ Step IDs are unique
...
3. Check that workflow file exists: `ls ../../packages/poem-core/tasks/test-prompt.yaml`
...
**Solution**:
1. Verify file exists first: `ls -l packages/poem-core/tasks/test-prompt.yaml`
2. Check file is not empty: `wc -l packages/poem-core/tasks/test-prompt.yaml` (should show ~523 lines)
3. Try cat command: `cat packages/poem-core/tasks/test-prompt.yaml | head -n 20`
...
**Sign-off**: ✅ Workflow definition accepted | ❌ Issues require fixes

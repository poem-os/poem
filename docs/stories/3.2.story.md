...
- [x] Task 1: Enhance Workflow Definition Structure (AC: 1, 7)
  - [x] Update `packages/poem-core/tasks/new-prompt.yaml` with complete workflow metadata
...
- [x] Task 8: Implement Mock Data Generation (AC: 5)
  ...
  - [x] Call `POST /api/mock/generate` with schema (placeholder - API to be implemented in Epic 8)
...
- `POST /api/mock/generate` - Epic 8 (Story 8.2)
...
The `POST /api/mock/generate` endpoint is planned for Epic 8, Story 8.2. For this story:
...
When Epic 8 is implemented, the workflow can be updated to use the API endpoint.
...
| Action | File |
|--------|------|
| Modified | `packages/poem-core/tasks/new-prompt.yaml` |

...
> **⚠️ Note:** Implementation superseded by **Story 3.7.1** (unified schema refactoring).
>
> This story was implemented with separate schema files (input + output), but the architecture design specified unified schema files with `input` and `output` sections in a single file. Story 3.7.1 corrected this implementation to align with the original design.
...
- Modified: `packages/poem-core/tasks/new-prompt.yaml` (updated STEP 1 guidance, STEP 4 template creation, STEP 5 dual schema generation, outputs section)
- Modified: `packages/poem-core/tasks/test-prompt.yaml` (updated STEP 4 to load output schema, STEP 6 to validate against output schema)
- Modified: `packages/poem-core/tasks/validate-prompt.yaml` (added new STEP 7 for output schema validation, renumbered subsequent steps)
...
**Patterns**:
- [API-First Heavy Operations](../../kdd/patterns/api-first-heavy-operations.md) - Schema validation delegated to runtime API instead of workflow logic
- [Unified Schema Structure](../../kdd/patterns/unified-schema-structure.md) - **Architecture violated**: Implemented separate files instead of unified structure (corrected in Story 3.7.1)

**Examples**: None (superseded by Story 3.7.1)

**Learnings**:
- [Architecture Validation Failure Story 3.7](../../kdd/learnings/architecture-validation-failure-story-3-7.md) - Story 3.7 implemented separate schema files, violating architecture spec; caught during QA review, fixed in Story 3.7.1
- [Test Expectation Maintenance](../../kdd/learnings/test-expectation-maintenance.md) - Dual schema change required updating 34+ test expectations; illustrated technical debt of schema format changes

**ADRs**:
- [ADR-001: Unified Schema Structure](../../kdd/decisions/adr-001-unified-schema-structure.md) - Decision to use unified schema format (violated here, corrected in Story 3.7.1)
- [ADR-003: API-First for Heavy Operations](../../kdd/decisions/adr-003-api-first-for-heavy-operations.md) - Validation logic delegated to API
...
- [Test Expectation Maintenance](../kdd/learnings/test-expectation-maintenance.md)
- [Architecture Validation Failure - Story 3.7](../kdd/learnings/architecture-validation-failure-story-3-7.md)

**Decisions (ADRs)**:
- [ADR-001: Unified Schema Structure](../kdd/decisions/adr-001-unified-schema-structure.md)

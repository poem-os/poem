...
```bash
cat packages/poem-core/tasks/new-prompt.yaml | head -40
```
...
- ✅ File exists at `packages/poem-core/tasks/new-prompt.yaml`
- ✅ Contains `id: new-prompt`
- ✅ Contains `version`, `author`, `lastUpdated` metadata
...
```bash
grep -E "^  - id:" packages/poem-core/tasks/new-prompt.yaml
```
...
- ✅ Contains 9 workflow steps
- ✅ Steps include elicitation (gather-*), action (create-*, generate-*), and output (complete)
...
```bash
grep -A2 "validation:" packages/poem-core/tasks/new-prompt.yaml
```
...
- ✅ `gather-purpose` has `required: true` and `minLength`
- ✅ `gather-inputs` has `required: true` and `minFields`
- ✅ `gather-name` has `required: true` and `pattern` for kebab-case
...
```bash
grep "stores:" packages/poem-core/tasks/new-prompt.yaml
```
...
- ✅ Key elicitation steps store their responses
- ✅ Action steps store their outputs
- ✅ Store names are descriptive and consistent
...
```bash
grep -A3 "id: generate-mock" packages/poem-core/tasks/new-prompt.yaml
```
...
- ✅ `generate-mock` step has `condition` block
- ✅ Condition checks `mockDataChoice` from previous step
- ✅ Matches pattern includes "yes", "1", or "y"
...
✅ Penny displays structured questions asking about:
...
✅ Penny asks for input fields with:
...
- [x] Test A: Workflow file exists with metadata ✅
- [x] Test B: Workflow has all 9 steps ✅
- [x] Test C: Elicitation steps have validation ✅
- [x] Test D: Steps use stores for data flow ✅
- [x] Test E: Conditional mock data generation ✅
...
**Overall Status**: 🔄 In Progress (Terminal Tests: 5/5 ✅ | Human Tests: 0/10 pending)

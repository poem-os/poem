...
- **Pattern**: [Field Mapper Pattern](../kdd/patterns/4-field-mapper-pattern.md) — Translates prompt-specific field names to workflow attribute names, decoupling prompt I/O contracts from workflow data model
- **Pattern**: [Workflow Chain Execution](../kdd/patterns/4-workflow-chain-execution.md) — Sequential prompt execution where each step's output feeds the next, with pause/resume via persistent workflow-data
- **Pattern**: [WorkflowData File Persistence](../kdd/patterns/4-workflowdata-file-persistence.md) — File-based JSON storage for workflow state enabling crash recovery, debugging, and resume capability
- **Decision**: [ADR-006: Field Mapper Architecture](../kdd/decisions/adr-006-field-mapper-architecture.md) — Adopts optional mapper objects on chain steps to translate field names without coupling prompts to workflow schema
- **Decision**: [ADR-008: File-Based Workflow Persistence](../kdd/decisions/adr-008-file-based-workflow-persistence.md) — Chooses file-based JSON persistence over database for workflow state, enabling portability and human-readable debugging

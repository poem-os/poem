...
**What to test**: Workflow file created at `packages/poem-core/tasks/refine-prompt.yaml`

**Command**:

```bash
ls -lh /Users/davidcruwys/dev/ad/poem-os/poem/packages/poem-core/tasks/refine-prompt.yaml
```

**Expected Output**:

```
-rw-r--r--  1 {user}  staff   {size}K {date} {time} /Users/davidcruwys/dev/ad/poem-os/poem/packages/poem-core/tasks/refine-prompt.yaml
```

**Validation**:

- ✅ File exists (no "No such file or directory" error)
- ✅ File size > 10KB (substantial workflow definition)
...
```
 ✓ tests/workflows/refine-prompt.test.ts (31 tests)
   ✓ Workflow YAML Structure
     ✓ should parse workflow YAML correctly
     ✓ should have required top-level fields
     ✓ should have pathResolution set to config
     ...
...
```bash
grep -A 5 "^id:" /Users/davidcruwys/dev/ad/poem-os/poem/packages/poem-core/tasks/refine-prompt.yaml
```
...
- ✅ `id` is "refine-prompt"
- ✅ `name` is "Refine Existing Prompt"
- ✅ `version` follows semantic versioning
- ✅ `author` is "POEM Framework"
- ✅ `lastUpdated` is valid date (YYYY-MM-DD)
...
```bash
grep "pathResolution:" /Users/davidcruwys/dev/ad/poem-os/poem/packages/poem-core/tasks/refine-prompt.yaml
```
...
- ✅ Contains `pathResolution: config`
- ✅ No `paths:` section exists (verify with `grep "^paths:" file` returns nothing)
...
```bash
grep "  - id:" /Users/davidcruwys/dev/ad/poem-os/poem/packages/poem-core/tasks/refine-prompt.yaml
```
...
- ✅ 10 steps total
- ✅ Step IDs match expected workflow sequence
- ✅ `select-prompt` (AC2: loads existing prompt)
- ✅ `load-template` (AC2, AC3: loads and displays)
- ✅ `render-current` (AC4: renders with data)
- ✅ `modification-method` + `apply-modifications` (AC5: modify template)
- ✅ `re-render` + `confirm-save` (AC6: save and compare)
- ✅ `track-iteration` (AC7: iteration history)
...
```bash
grep -A 10 "id: track-iteration" /Users/davidcruwys/dev/ad/poem-os/poem/packages/poem-core/tasks/refine-prompt.yaml | head -15
```
...
- ✅ Step exists with id `track-iteration`
- ✅ Type is `action`
- ✅ Has `action: optional` or marked optional in instruction
- ✅ Conditional execution based on `changesSaved`
...
- ✅ File exists
- ✅ File size > 5KB (substantial test coverage)
...
- ✅ Exactly 31 test cases
- ✅ Tests cover: YAML structure, step validation, AC coverage, path resolution, outputs, metadata
...
```
/Users/davidcruwys/dev/ad/poem-os/poem/packages/poem-core/tasks/refine-prompt.yaml
```
...
```bash
cd /Users/davidcruwys/dev/ad/poem-os/poem
grep "pathResolution:" packages/poem-core/tasks/refine-prompt.yaml
```

...
- [x] Task 1: Create agent definition file structure (AC: 1, 2)
  ...
  - [x] Add BMAD header comment: `<!-- Powered by BMAD™ Core -->`
...
- [x] Task 8: Create Claude Code slash command wrapper (AC: 6)
  - [x] Create `.claude/commands/poem/agents/penny.md`
...
- [x] Task 9: Manual testing via Claude Code (AC: 6, 7)
  - [x] Test: `/poem/agents/penny` activates agent
...
packages/poem-core/
├── agents/
│   └── prompt-engineer.md         ← CREATE: Agent definition
│
├── workflows/                      (Future stories - placeholder references only)
│   ├── new-prompt.yaml
│   ├── refine-prompt.yaml
│   ├── test-prompt.yaml
│   └── validate-prompt.yaml
│
├── skills/                         (Future stories - placeholder references only)
│   ├── check-my-prompt.md
│   ├── preview-with-data.md
│   └── generate-schema.md
│
├── data/
│   ├── poem-principles.md          ← CREATE: Core principles
│   └── prompt-best-practices.md    ← CREATE: Best practices
│
└── core-config.yaml                (Already exists from Epic 1)

.claude/commands/poem/
└── agents/
    └── prompt-engineer.md          ← CREATE: Slash command wrapper
```
...
- `.claude/commands/poem/agents/penny.md` - Slash command wrapper
...
**Patterns**: None (basic agent setup story)
...
- Coding Standards: ✓ kebab-case file naming, BMAD structure followed
- Project Structure: ✓ Files in correct locations per unified-project-structure.md
- Testing Strategy: ✓ Manual testing via Claude Code appropriate for agents
...
|----|-------------|--------|----------|
| 1 | Agent definition file | ✓ PASS | `packages/poem-core/agents/prompt-engineer.md` exists |
| 2 | BMAD structure | ✓ PASS | YAML config with all required sections |
| 3 | Persona defined | ✓ PASS | Penny, role, style, 7 principles |
| 4 | Commands listed | ✓ PASS | help, new, refine, test, validate, exit |
| 5 | Loads knowledge base | ✓ PASS | STEP 4 loads poem-principles.md |
| 6 | Slash command works | ✓ PASS | `.claude/commands/poem/agents/penny.md` |
| 7 | Greets and displays commands | ✓ PASS | STEP 5 in activation-instructions |
...
Gate: **PASS** → docs/qa/gates/3.1-prompt-engineer-agent.yml

### Recommended Status

✓ **Ready for Done** - All acceptance criteria met, all terminal tests passed, implementation follows established patterns with no issues found.
...
- [Slash Command Source Duplication](../kdd/learnings/slash-command-source-duplication.md)

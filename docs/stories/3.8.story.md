...
**Patterns**:
- [Config Service Single Source of Truth](../../kdd/patterns/config-service-single-source-of-truth.md) - Multi-workflow paths managed by config service, not hardcoded
- [Workflow-Scoped Resource Management](../../kdd/patterns/workflow-scoped-resource-management.md) - Phase 1 multi-workflow implementation demonstrates workflow-specific resource isolation

**Examples**:
- [Multi-Workflow Setup](../../examples/multi-workflow-setup/README.md) - Demonstrates creating multiple workflow directories (youtube-launch-optimizer, nano-banana) with isolated prompts/schemas/mock-data

**Learnings**:
- [Workflow Persistence Edge Cases](../../kdd/learnings/workflow-persistence-edge-cases.md) - Discovered 3 edge cases during Phase 1 implementation: missing config file, invalid JSON, directory creation failures; all handled gracefully

**ADRs**:
- [ADR-002: Multi-Workflow Architecture](../../kdd/decisions/adr-002-multi-workflow-architecture.md) - Formalized multi-workflow design with Phase 1 (filesystem isolation) and Phase 2 (workflow orchestration/composition)
...
- **Pattern**: [Workflow-Scoped Resource Management](../kdd/patterns/workflow-scoped-resource-management.md) — Isolates prompts, schemas, and mock-data under per-workflow directories; switching context changes all path resolution without restarting
- **Pattern**: [Config Service Single Source of Truth](../kdd/patterns/config-service-single-source-of-truth.md) — Single TypeScript config service resolves all workspace paths; no path duplication in tasks or agents
- **Learning**: [Workflow Persistence Edge Cases](../kdd/learnings/workflow-persistence-edge-cases.md) — Edge cases in workflow state persistence (missing state file, parse errors, invalid workflow name) require graceful fallback to null rather than crashes
- **Decision**: [ADR-002: Multi-Workflow Architecture](../kdd/decisions/adr-002-multi-workflow-architecture.md) — Adopts named workflow directories over a flat workspace to support multiple independent prompt collections within one POEM installation

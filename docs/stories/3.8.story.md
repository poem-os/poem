...
- [ ] Task 11: Manual Testing (AC: ALL)
  ...
  - [ ] Test config loading with multi-workflow structure
  - [ ] Activate Penny: `/poem/agents/penny`
  - [ ] Test `*workflows` command (lists workflows correctly)
  - [ ] Test `*context` command (shows current workflow)
  - [ ] Test `*switch youtube-launch-optimizer` (switches successfully)
  - [ ] Test `*list` (shows only youtube-launch-optimizer prompts)
  - [ ] Create test prompt in youtube-launch-optimizer
  - [ ] Test `*switch nano-banana` (switches successfully)
  - [ ] Verify previous prompt not visible in nano-banana
  - [ ] Create test prompt in nano-banana
  - [ ] Test persistence: restart agent, verify currentWorkflow persists
  - [ ] Test reference path display in `*context` command

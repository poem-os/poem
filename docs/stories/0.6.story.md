...
- ✅ [Missing Dependency Resolution](../kdd/learnings/testing-missing-dependency-kdd.md) - Zod imported but not declared as explicit dependency, causing 10 test failures. Single fix (add zod@^3.23.0 to package.json) resolved all failures (75-88% failure reduction).

**Related KDD** (referenced during story):
- [Testing Infrastructure Challenges](../kdd/learnings/testing-infrastructure-challenges-kdd.md) (created during Story 1.11, Story 0.6 fixed 10 of those failures)

**Patterns Created**:
- None (dependency fix too simple to warrant pattern documentation. Will promote to pattern if recurs 3+ times)
...
**Knowledge Extraction Status**: ✅ Curated by Lisa (Librarian) on 2026-01-29

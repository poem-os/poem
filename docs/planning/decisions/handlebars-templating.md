...
{{#if formal}}
  Dear
  {{#if recipient.title}}{{recipient.title}} {{/if}}{{recipient.last_name}},
{{else}}
  Hi
...
```javascript
{
  questions: [
    { question: "How did they seem?", answer: "Tired and quiet" },
    {
      ...
      answer: "Yes, 3 other participants",
    },
  ];
}
```
...
```handlebars
Participant:
{{formatName participant style="preferred"}}
Reporter:
{{formatName reporter style="formal"}}
```
...
**Participant**:
{{formatName participant style="full"}}
**Date/Time**:
{{formatDate incident.datetime format="long"}}
**Location**:
{{#if location.building}}{{location.building}} - {{/if}}{{location.room}}
**Reporter**:
{{formatName reporter style="formal"}}

...
**Technology Stack:** YAML task definitions (BMAD v4 pattern)

**Tasks:**

| Task            | File                         | Steps                                                        |
| --------------- | ---------------------------- | ------------------------------------------------------------ |
| New Prompt      | `tasks/new-prompt.yaml`      | Gather purpose → Create template → Generate schema → Preview |
| Refine Prompt   | `tasks/refine-prompt.yaml`   | Load → Test → Identify issues → Update → Repeat              |
| Test Prompt     | `tasks/test-prompt.yaml`     | Select data source → Render → Report                         |
| Validate Prompt | `tasks/validate-prompt.yaml` | Check syntax → Validate schema → Check helpers → Report      |
| Deploy Prompt   | `tasks/deploy-prompt.yaml`   | Validate → Test connection → Publish → Confirm               |
| Add Helper      | `tasks/add-helper.yaml`      | Describe need → Generate code → Register → Test              |

...
When testing the new agent via `/poem/agents/penny`, it failed to load:
...
3. **Development Convenience**
   - Option A: Symlink `.poem-core/` → `packages/poem-core/` during development
...
3. **Slash Command Wrapper**:
   - `.claude/commands/poem/agents/penny.md` - verify `IDE-FILE-RESOLUTION` works
